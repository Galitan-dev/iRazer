	...
	static discover() {
		if (!this.razerDevices)
			this.razerDevices = addon.getAllDevices().map(function (device) {
				const properties = RazerDevice.devicesProperties.find(
					({ productId }) => productId === device.productId
				);
				switch (properties.type) {
					case 'keyboard':
						return new RazerKeyboardDevice(
							properties,
							device.internalDeviceId
						);
					default:
						return new RazerDevice(properties, device.internalId);
				}
			});
...
class RazerKeyboardDevice extends RazerDevice {
	...
	setMode(mode) {
		switch (mode) {
			case 'wave':
				return addon.kbdSetModeWave(this.internalId, 1);
			case 'spectrum':
				return addon.kbdSetModeSpectrum(this.internalId);
			case 'custom':
				return addon.kbdSetModeCustom(this.internalId);
			default:
				return addon.kbdSetModeNone(this.internalId);
		}
	}
